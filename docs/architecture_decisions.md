...
## ADR-148：随机跨轮与显式退出随机使用独立原生证据合同

Phase7J13B。J13A精确7b28a16的Android根测试已真实通过，才新增独立三曲随机测试；不改生产循环算法、不扩大A或旧Windows探针的成功合同。固定Random种子通过生产根的既有randomIndex注入点控制选择，不直接生成/注入原生事件或序列。自然观测首轮与新轮，确认SQLite队列未按随机顺序重排；在新轮首曲关闭随机，要求同曲继续前进、旧预载尾部不再播放，随后自然按应用队列进入不同于旧随机尾部的下一曲。最后关闭自动继续，观察本曲完成及有界无重启。仅测试自有低幅WAV与文件SQLite，不操作用户数据或音频设置。

独立结果要求固定种子、精确SHA、唯一成功测试、原生批次/索引/轮次/时钟、根与持久化顺序、策略变化后的原生完成与单曲进度、清理完成；失败与缺证据拒绝。默认关闭的Android-only输入与旧模式互斥，先执行既有WAV/content，再运行本测试及宿主门禁。A保持原文件与合同不变。真实设备执行与本地合同回归、双平台构建分别记账，不把随机状态证据等同声学、Windows播放或Phase7完成。

## ADR-147：根原生验证独立于引擎探针，成功证据必须等待真实资源清理

Phase7J13A。沿用既有唯一PlaybackController、JustAudioEngine及Drift仓库，不新增生产播放逻辑。独立Android入口只使用临时目录生成的两项WAV与文件SQLite，观测同一原生批次q0→q1→q0、根元数据与持久化游标、真实时钟确认的历史去重；新轮关闭自动继续后观察自然完成及有界无重启，再关闭根/引擎/数据库并重新打开同一隔离数据库验证不自动播放。关闭根不等于关闭借用引擎，两者分别等待，最后只删除本探针创建的临时目录。

严格结果只包含固定测试身份、原生和根进度及断言后的布尔事实，不输出路径、SQL、历史ID或插件错误原文。等待allTestsPassed（含每测试teardown）后才输出最终白名单结果，宿主独立校验精确SHA、恰好一个成功测试和完整指标；缺失、重复输出、失败、超时、跳过或不匹配均拒绝。新增默认关闭的Android专用workflow输入，旧单源/序列/Windows Profile合同不变，诊断不进入生产main、不创建Release。成功仅限模拟器状态/时钟/持久化；不关闭Windows实际播放、声学、后台或Phase7整体出口。
