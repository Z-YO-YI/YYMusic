...
当前增量Phase6H6：音乐库与专辑/艺人详情可从歌曲菜单添加到已有自定义歌单。
选择器在根原生路由使用Phone底部面板、Tablet/Windows对话框，状态高于可替换Shell；
显式原生IME筛选、元数据20→200有界前缀、同名独立ID、失效来源引用和重复条目保留。
旧快照/草稿/菜单/关闭回调不能误添加或误关新面板；覆盖路由禁止动作，返回恢复焦点。
借用唯一根writer，关闭后已接受写入继续排空，失败保留脱敏提示；不复制文件、不改变现有播放。
718 Flutter（81 Golden：3新、12旧更新、66旧未改）/92 Node、严格分析通过；
分支`codex/playlist-add-picker`，Android预检/精确云端状态见[Phase6H6报告](phase_6h6_playlist_add_picker_report.md)。
前置Phase6H5 `4847f29`、Draft PR #50两组GitHub checks/Android/Windows成功，未合并。
下一步仍需创建并添加、播放全部/随机、系统歌单和超过200条内容浏览策略，不代表整个Phase6完成。
