...
新增6张130%字体基线：Phone浅色/空库、Tablet竖屏深色/横屏来源、Windows宽深色/窄来源。
使用打包字体/SVG，逐张查看；另外只更新受首页替换影响的9张原有整屏基线并全部查看，
其余34张旧基线不变。最终49张以非更新模式精确比较通过。窄宽、滚动后来源/历史、禁用与空态无溢出。
默认测试Graph未注入数据合同时显示错误，而实际生产新库为空；不把这些错误截图当正常用户库。
Hero采用App.tsx后置28圆角、双阴影、标题字距与新版图标。原生圆盘插图为简化适配，封面为明确占位；
这不是HTML浏览器截图对比或像素一致性验收。Phase2参考采集缺口保留，未绕过file:导航安全拒绝。
