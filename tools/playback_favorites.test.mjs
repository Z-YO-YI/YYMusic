...
test('current favorite root borrows playback and collection with explicit start and shutdown barrier', () => {
  ...
  assert.match(graph, /await playback.initialize\(\);\s*if \(_closeFuture != null\) return;\s*await sleepPersistence.initialize\(\);\s*if \(_closeFuture != null\) return;\s*await continuationPersistence.initialize\(\);\s*if \(_closeFuture == null\) \{\s*playbackFavorite.start\(\)/);
