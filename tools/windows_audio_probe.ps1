...
function Get-SequenceMode($Record) {
    $property = $Record.PSObject.Properties['includeSequence']
    if (-not $property) { return $false }
    if ($property.Value -isnot [bool]) { throw 'Invalid sequence probe mode' }
...
if ($Mode -eq 'Run') {
    ...
    if ($IncludeSequence) {
        ...
        $sequenceMetrics = & (Join-Path $PSScriptRoot 'native_sequence_result.ps1') -Record $sequenceRecord -ExpectedCommit $nativeCommit
        ...
        [ordered]@{ passed = $true; sourceCommit = $manifest.sourceCommit; nativeCommit = $nativeCommit;
            runtimeMode = $RuntimeMode; includeSequence = $true; testCount = 1; renderEndpoints = $renderCount;
            exitCode = $probeExit; sequenceMetrics = $sequenceMetrics } | ConvertTo-Json -Depth 5 -Compress
        return
    }
