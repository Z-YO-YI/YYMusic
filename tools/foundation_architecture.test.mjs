...
test('Home uses root playback and confirms destructive history action', () => {
  ...
  assert.match(graph, /home\.close,\s+appearanceSettings\.close,\s+search\.close,\s+libraryController\.close,\s+localMusic\.close,\s+catalogDetails\.close,\s+playlists\.close,\s+playlistAdds\.close,\s+playlistContents\.close,\s+systemPlaylists\.close,\s+lyricsController\.close,\s+queue\.close,\s+playbackFavorite\.close,\s+playback\.close/);
...
test('shared Shell presenter maps root playback and guards queued seek identity', () => {
  ...
  assert(!/onOpenQueue:|onOpenFullscreen:/.test(shell));
  for (const path of [
