...
test('Home uses root playback and confirms destructive history action', () => {
  ...
  assert.match(graph, /home\.close,\s+search\.close,\s+libraryController\.close,\s+catalogDetails\.close,\s+playlists\.close,\s+playlistContents\.close,\s+playback\.close/);
