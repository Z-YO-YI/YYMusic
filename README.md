...
当前阶段：**Android + Windows · Phase 6G2 专辑／艺人原生详情**。音乐库专辑/艺人入口、连续详情返回栈、三套布局、真实分页/错误重试和根播放器动作已接入；修复跨600断点丢会话、横竖屏滚动归零和URL替换时误复用旧来源三项问题。506项Flutter（67张Golden）、77项Node、严格分析及本地Android Debug通过；本批GitHub精确提交构建状态见报告/对应Draft PR。前置Phase6G1 `5508cf4` 两组GitHub checks/Android/Windows均成功。搜索详情入口、详情收藏菜单、歌单编辑、导入/REST、完整播放器及发布仍未完成，尚不是完整可用或上线版本。
...
正式搜索页输入曲目、专辑、艺术家或来源名，完成输入法合成后防抖查询。点击“搜索目录”只搜索；Enter搜索并播放当前可见的首个可用曲目，优先本地，专辑/艺术家筛选不播放隐藏结果。六个数据区域分别分页/重试，每页20条、每区最多读取200条后提示缩小查询。仅显式提交才保存本机搜索历史；清除历史需确认，不删除音乐或清空查询。Windows Ctrl+K会返回输入位置并聚焦，输入空格不触发播放；离开搜索会撤销尚未执行的播放意图。搜索结果中的专辑/艺术家详情入口及实时联网搜索尚未接入，结果与占位状态有明确说明。
