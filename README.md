...
当前阶段：**Android + Windows · Phase 2G 跨平台弹层原语**。主题/字体/44 SVG、双平台导航、受控播放器、Context Menu/Dialog/Phone Bottom Sheet/Toast及共享Gallery已有。APK只由GitHub Actions构建；Phase2G实现提交的push、PR、手动双平台构建与云端APK已完成独立复核。不是完整音乐客户端，也不代表整个Phase2完成；真实播放、数据库、业务Overlay编排、窗口Gateway和正式业务页尚未接入。

Phase2G本地门禁已完成：75个Dart文件格式无变更、严格分析0问题、104项Flutter与23张Windows宿主Golden、28项Node、五份设计指纹及24份ZIP逐字节复核全部通过。云端状态仍必须按目标commit另行验证。
...
需要APK时，在[GitHub Actions](https://github.com/Z-YO-YI/YYMusic/actions/workflows/foundation.yml)对目标分支手动运行工作流；成功后从该次运行生成的私有草稿Release下载。Phase2G实现提交a56d4aa已由[运行33466396076](https://github.com/Z-YO-YI/YYMusic/actions/runs/33466396076)生成并完成[草稿Release](https://github.com/Z-YO-YI/YYMusic/releases/tag/untagged-01936e0c98443e18e11d)复核，APK SHA-256为`836e97c46aef2ed0036aaec085b8fb6113beeb2fe363749a7dfa3e817f648a88`。必须确认Android任务、Release标签、metadata完整commit和SHA256SUMS一致；普通push/PR只验证构建，不创建下载产物。旧本机APK不是本批云端产物，APK不提交Git源码。证据见[Phase2G报告](docs/phase_2g_overlay_primitives_report.md)，详情和临时Debug签名限制见[构建说明](docs/github_apk_build.md)。

无已连接真机/模拟器或本机Windows原生运行验收证据，构建成功不等于已安装运行。23张组件/原生Shell Golden使用打包字体、Flutter 3.47.2 / Windows测试宿主，精确像素比较；Linux明确跳过这23张，运行其余81项测试，Windows CI执行Golden。只在审查视觉变更后对指定测试使用`--update-goldens`，日常测试不得更新基线。
...
仓库：[Z-YO-YI/YYMusic](https://github.com/Z-YO-YI/YYMusic)。当前开发分支`feat/cross-platform-overlay-primitives`基于已拉取并同步的`feat/cross-platform-player-surfaces@6ecbbd8`，未在main/master开发。此前阶段提交保留；合并前需审核完整分支差异。经用户明确授权已恢复临时GitHub API访问，不读取现有Git凭据、不持久化访问令牌；Git提交/推送和云端产物状态以每次交付时实际核验为准。
