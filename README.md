...
当前阶段：**Android + Windows · Phase 6G1 专辑／艺人详情状态层**。来源隔离摘要、独立曲目/专辑分页、刷新取消及根会话关闭等待已实现；同时修正音乐库/搜索短中间页导致总量超过200条的问题。479项Flutter（原61张Golden未改）、75项Node、严格分析及本地Android Debug通过；本批云端状态见报告/对应Draft PR。前置Phase6F `b72413e` 两组GitHub Android/Windows、Golden和原生窗口检查均成功。现有音乐库五分类和曲目动作保持；详情路由/三端页面将在Phase6G2接入，歌单编辑、导入/REST、完整播放器及发布仍未完成，尚不是完整可用或上线版本。
...
外观仅在本次运行保留，重启恢复默认。预览页的点击/收藏/进度只是标注清楚的本页示例状态。正式首页、搜索和底栏接根数据库/播放器；音乐库、设置主体仍是骨架（开源许可入口已可用）。首页右上调色板进入设计预览，刷新按钮重新读取目录与来源/历史；来源状态仅为Repository保存值，不代表重新联网测试。目前没有可操作的曲库导入流程，因此默认新安装显示真实空库。

正式搜索页输入曲目、专辑、艺术家或来源名，完成输入法合成后防抖查询。点击“搜索目录”只搜索；Enter搜索并播放当前可见的首个可用曲目，优先本地，专辑/艺术家筛选不播放隐藏结果。六个数据区域分别分页/重试，每页20条、每区最多读取200条后提示缩小查询。仅显式提交才保存本机搜索历史；清除历史需确认，不删除音乐或清空查询。Windows Ctrl+K会返回输入位置并聚焦，输入空格不触发播放；离开搜索会撤销尚未执行的播放意图。专辑/艺术家详情及实时联网搜索尚未接入，结果与占位状态有明确说明。
