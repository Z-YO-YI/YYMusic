...
切歌、覆盖页面、改变窗口尺寸或离页后，旧手势不能跳转新曲目。封面明确显示“暂无封面”兜底，暂停时轻微缩小；开启减少动态则不缩放。右上角歌词图标打开独立原生歌词页。真实封面读取、系统全屏/F与独立队列编辑仍待后续 Phase 7；不提供模拟入口。默认新安装为空库，Phase 8 导入/扫描尚未接入。
...
尚无歌词、未选歌曲和读取失败有独立提示；重试歌词不停止音频，也不显示私有异常。当前为固定深色纯色兜底，不冒充封面提色；未接收藏、OS全屏、Android沉浸UI及LRC导入，不以页面可打开宣称这些能力已完成。新安装仍需后续 Phase 8 的真实导入功能才能建立本地音乐库。
